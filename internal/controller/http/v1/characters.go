...
func (r *charactersRoutes) characters(c *fiber.Ctx) error {
	...
	if c.Query("count") == "" {
		...
		r.l.Error(err, "http - v1 - characters - count")
		return errorResponse(c, fiber.StatusBadRequest, "Query parameter 'count' is not an integer.")
	}
	if count <= 0 {
		r.l.Error("count is negative or zero", count, "http - v1 - characters - count")
	...
	if err != nil {
		r.l.Error(err, "http - v1 - characters")
...
func (r *charactersRoutes) character(c *fiber.Ctx) error {
	...
	if _, err := strconv.Atoi(id); err == nil {
		r.l.Error(errors.New("route parameter 'id' is not a string"), "http - v1 - character - id")
	...
	if err != nil {
		r.l.Error(err, "http - v1 - character")
...
func (r *charactersRoutes) characterImages(c *fiber.Ctx) error {
	id := c.Params("id")
	if _, err := strconv.Atoi(id); err == nil {
		r.l.Error(errors.New("route parameter 'id' is not a string"), "http - v1 - characterImages - id")
		return errorResponse(c, fiber.StatusBadRequest, "Route parameter 'id' is not a valid id.")
	}
	...
	if err != nil {
		r.l.Error(err, "http - v1 - characterImages")
		return errorResponse(c, fiber.StatusInternalServerError, "Internal server error")
	}
	count := len(images)
